...
const TILE_SIZE = 64;
const DISPATCH_PRIMITIVE_PACKAGE_SIZE = 8;
const VP_PROJ_TRANSFORM_PAKCAGE_SIZE = 8;
const RASTERIZE_PRIMITIVE_PACKAGE_SIZE = 1;

type Pixel_statistic	struct {
	 ps_invocations		uint64
	 backend_input_pixels	uint64
}

type Drawing_triangle_context struct{
	aa_z_offset		*float64
	tri_info 		*Triangle_info
	pixel_stat		*Pixel_statistic
}
type vs_output_pool []shader.Vs_output
...
type drawing_shader_context struct{
	cpp_ps		*shader.Cpp_pixel_shader
	ps_unit		*shader.Pixel_shader_unit
	cpp_bs		*shader.Cpp_blend_shader
};

type rasterize_multi_prim_context struct {
	
	sorted_prims	*[]uint32
	tile_vp			*Viewport
	pixel_stat		*Pixel_statistic
	shaders			drawing_shader_context
}

type rasterize_prim_context struct {
...
// Status per drawing.
	num_vs_output_attributes_	uint32

	vert_cache_					*shader.Vertex_cache
	host_						*Host
	frame_buffer_				*Framebuffer

	state_						*Raster_state
	ps_proto_					*shader.Pixel_shader_unit
	cpp_vs_						shader.Cpp_vertex_shader
	cpp_ps_						shader.Cpp_pixel_shader
	cpp_bs_						shader.Cpp_blend_shader
	vp_							*Viewport
	target_vp_					*Viewport
	target_sample_count_		uint32
	full_mask_					uint64
	quad_full_mask_				uint64
	vso_ops_					*shader.Vs_output_op
	has_centroid_				bool
	prim_count_					uint32
	prim_reorderable_			bool								// Primitives could be reordered to rendering.

	pipeline_stat_				*Async_object
	internal_stat_				*Async_object
	pipeline_prof_				*Async_object

	//type	acc_ia_primitives_		accumulate_fn<uint64_t>::
	//type	acc_cinvocations_		accumulate_fn<uint64_t>::
	//type	acc_cprimitives_		accumulate_fn<uint64_t>::
	//type	acc_ps_invocations_		accumulate_fn<uint64_t>::
	//type	acc_backend_input_pixels_		accumulate_fn<uint64_t>::
	//
	...
	prim_										Prim_type
	prim_size_									uint32
	samples_pattern_[MAX_NUM_MULTI_SAMPLES]		[2]int
 ...
 **************************************************/
func rasterize_line( ctx *rasterize_prim_context){
